...
export default function ForgotPasswordFlow({ children }: ForgotPasswordFlowProps) {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      setCooldownLeft((previous) => Math.max(0, previous - 1));
    }, 1000);
      ...
      {showSuccessBanner ? (
        <div className="mt-6 space-y-5 border-t border-gray-200 pt-6">
          <div className="flex gap-3 rounded-lg bg-[#E8F5E9] px-4 py-3 text-left">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#15803d]" aria-hidden>
          ...
          {cooldownLeft > 0 ? (
            <div className="space-y-4 text-center">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-mid">Didn&apos;t receive the email?</p>
              <div
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#eef1f6] py-3.5 text-sm font-medium text-slate-mid"
                role="status"
                aria-live="polite"
              >
                <Timer className="h-[18px] w-[18px] shrink-0 text-slate-mid" strokeWidth={1.75} aria-hidden />
