...
import type { ProjectCardItem } from "../types/project-card-item";
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        <div key={project.id} className="relative">
          ...
          >
            <article className="aspect-square w-full flex flex-col justify-between rounded-xl border border-slate-200 bg-[#f7f8fa] p-4 pt-10 transition hover:border-slate-300 hover:shadow-sm">
              <div>
                <h2 className="text-sm font-semibold text-[#0f172a] line-clamp-2">{project.name}</h2>
                <p className="mt-1 text-xs text-slate-500 line-clamp-3">{project.description}</p>
              </div>

              <div className="mt-3 border-t pt-2 flex justify-between items-center">
                <span className="text-[9px] uppercase text-slate-400">Created</span>
                <span className="text-xs text-slate-600">{project.createdAtLabel}</span>
              </div>
            </article>
          </Link>
          <Link
      ...
      >
        ...
        <span className="text-sm font-semibold uppercase tracking-[0.14em] text-[#374151]">Add project</span>
      </Link>
    </div>
  );
}

export { ProjectsErrorState };
