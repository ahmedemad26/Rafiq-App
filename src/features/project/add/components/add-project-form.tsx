...
import { AlertCircle, Check, Lightbulb, Plus } from "lucide-react";
import { type SubmitHandler, useForm } from "react-hook-form";
import { Form, FormControl, FormField, FormItem, FormMessage } from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { createProjectSchema, type CreateProjectValues } from "@/lib/schemes/products-shema/add-project.shema";
...
import { useAddNewProject } from "../hooks/use-add-new-project";
      ...
      footer={
        <div className="flex gap-3 rounded-b-2xl bg-[#E8EEF8] px-6 py-2.5 sm:px-8">
          ...
          <p className="text-xs leading-snug text-slate-600 sm:text-[13px] sm:leading-relaxed">
            <span className="font-semibold text-slate-700">Pro Tip:</span> You can invite project members and assign epics.
          </p>
        </div>
      }
    >
          ...
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#DCE8FF]" aria-hidden>
            <span className="relative flex h-7 w-7 items-center justify-center">
              ...
              <span className="absolute -right-0.5 -top-0.5 flex h-3 w-3 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-slate-200/80">
                <Plus className="size-1.5 text-[#003380]" strokeWidth={4} />
              </span>
            </span>
          </div>
