...
import type { TaskDetailsDialogProps } from "@/lib/types/features/tasks/task-details-dialog";
import { TaskDetailsLoadingState } from "@/features/project/tasks/shared/components/task-details-states";
import { TaskDetailsMessageState } from "@/features/project/tasks/shared/components/task-details-message-state";
...
import { formatDate } from "../utils/task-details-utils";
import { useTaskDetailsDialogState } from "../hooks/use-task-details-dialog-state";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {isPending ? <TaskDetailsLoadingState /> : null}

        {!isPending && isError ? <TaskDetailsMessageState message="Failed to load task details" /> : null}

        {!isPending && !isError && !task ? <TaskDetailsMessageState message="Task not found" /> : null}
        ...
        {!isPending && !isError && task ? (
          <div className="grid min-h-[620px] grid-cols-1 divide-y sm:grid-cols-[1.75fr_1fr] sm:divide-x sm:divide-y-0">
            <section className="flex min-h-full flex-col">
              ...
              <div className="mt-auto hidden items-center justify-between border-t border-slate-200 bg-[#F4F7FF] px-6 py-3 sm:flex">
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-[#F4F7FF] px-4 py-3 sm:hidden">
              <button
                type="button"
                className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-slate-700"
              >
