...
export function AppNavbar() {
  ...
  const userRole = session?.user?.user_metadata?.department || session?.user?.role || "Member";
      ...
      <div className="flex h-full flex-1 items-center justify-end px-4">
        <div className="flex items-center gap-3">
          <div className="hidden text-right leading-tight md:block">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[1px] text-[#1D4ED8]">{userRole}</p>
          </div>
