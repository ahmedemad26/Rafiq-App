...
interface MyStatisticsStatusChartProps {
  totalTasks: number;
...
export default function MyStatisticsStatusChart({
  ...
  return (
    <SectionCard
      ...
      titleClassName="text-[18px] font-semibold text-[#1A1A2E]"
      className="rounded-[20px] px-9 py-8"
              ...
              <div key={entry.key} className="space-y-[7px]">
                <div className="flex items-center justify-between">
                  <span className="inline-flex items-center gap-[9px]">
                    ...
                    <span className="text-[14px] leading-none font-medium text-[#2D3150]">{formatLegendLabel(entry.key)}</span>
                  </span>
                  <span className="text-[14px] leading-none font-semibold text-[#1A1A2E]">{entry.value}</span>
                </div>
                <div className="h-1 rounded-full bg-[#EEF0F6]">
                  <div className={cn("h-1 rounded-full")} style={{ width: `${Math.min(100, entry.percent)}%`, backgroundColor: entry.color }} />
                </div>
              </div>
            ))
          ) : (
