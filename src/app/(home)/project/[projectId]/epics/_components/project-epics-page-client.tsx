...
export default function ProjectEpicsPageClient({ projectId }: ProjectEpicsPageClientProps) {
  ...
  const epics = useMemo(() => {
    if (isMobile) {
      return infiniteResult?.pages.flatMap((page) => page.data) ?? [];
    }
    return pagedResult?.data ?? [];
  }, [infiniteResult?.pages, isMobile, pagedResult?.data]);

  const totalCount = isMobile
    ? (infiniteResult?.pages[0]?.totalCount ?? epics.length)
