...
import { formatRangeLabel, parseDate, toDateInputValue } from "./my-statistics.utils";
...
type MyStatisticsRangePickerProps = {
  startDate: string;
  endDate: string;
  open: boolean;
  onToggleOpen: () => void;
  ...
  return (
    <div className="relative flex min-w-0 items-center gap-2.5">
      <button
        ...
        className="inline-flex size-7 items-center justify-center rounded-md text-[#4A4F72] hover:bg-[#DDE0EF]"
      ...
      <button
        ...
        className="whitespace-nowrap rounded-md px-1 py-0.5 text-sm font-semibold text-[#1A1D3A] hover:bg-[#DDE0EF]"
      >
        ...
        <div className="absolute left-0 top-[calc(100%+12px)] z-30 w-[330px] rounded-xl border border-[#E2E6F2] bg-white p-4 shadow-[0_12px_30px_rgba(0,0,0,0.12)]">
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="inline-flex size-7 items-center justify-center rounded text-[#4A4F72] hover:bg-[#EEF0F8]"
              >
                <ChevronLeft className="size-4" />
              </button>
              <button
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            {WEEKDAY_HEADERS.map((day) => (
              <span key={day} className="py-1 text-[12px] font-semibold text-[#9BA3B8]">
                {day}
              </span>
            ))}
            {days.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`h-9 rounded-md text-[18px] ${
                    !currentMonth
                      ? "text-[#C5CEDF]"
                      : startActive || endActive
                        ? "bg-[#C8D8F3] font-semibold text-[#1D4ED8]"
                        : active
                          ? "bg-[#E3ECFA] text-[#1D4ED8]"
                          : "text-[#1A2A49] hover:bg-[#EEF0F8]"
                  }`}
                >
                  {day.getDate()}
                </button>
          ...
          <div className="mt-3 border-t border-[#E8ECF4] pt-3">
            <div className="flex items-center justify-between">
              <button
                type="button"
                onClick={onToggleOpen}
                className="h-10 rounded-md px-5 text-[20px] font-medium text-[#526180] hover:bg-[#EEF0F8]"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={() => onApplyRange(draftStartDate, draftEndDate)}
                className="h-10 rounded-md bg-[#1453B8] px-6 text-[18px] font-semibold text-white hover:bg-[#0f47a3]"
              >
                Apply Range
              </button>
            </div>
